...
RegisterNetEvent('jd_crafting:giveitems')
AddEventHandler('jd_crafting:giveitems', function(craftingType, craftingOption)
    local src = source
    ...
    if hasRequiredItems then
        local result = lib.callback.await('jd_crafting:serverprogress', src, text, time)
    ...
    local embeds = {
        {
            ...
            ["color"] = 56108,
            ["footer"] = {
                ["text"] = "jd_crafting " .. os.date('%H:%M - %d. %m. %Y', os.time()),
            },
        }
    }
